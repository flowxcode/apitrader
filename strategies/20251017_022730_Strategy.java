...
public class Strategy implements IStrategy {
    ...
    private final double amount = 1.0; // Increased for more leverage
    ...
    private final int advisorTrendWeight = -3; // -3 for this strategy (bearish bias)
    ...
    public void onBar(Instrument instrument, Period period, IBar askBar, IBar bidBar) throws JFException {
        if (instrument == Instrument.EURUSD && period == Period.ONE_HOUR) {
            ...
            // Bias based on advisor_trend_weight: -10 to +10, maps to probBuy 0 to 1
            double probBuy = (advisorTrendWeight + 10.0) / 20.0;
            IEngine.OrderCommand cmd = (rand.nextDouble() < probBuy) ? IEngine.OrderCommand.BUY : IEngine.OrderCommand.SELL;
            ...
            String comment = "trend_weight: " + advisorTrendWeight;
            engine.submitOrder(label, instrument, cmd, amount, entryPrice, slippage, slPrice, tpPrice, goodTillTime, comment);
            console.getOut().println("Placed random " + cmd + " order: " + label + " SL:" + slPrice + " TP:" + tpPrice + " ATR:" + hourlyAtr + " Weight:" + advisorTrendWeight);
        }
    }
}