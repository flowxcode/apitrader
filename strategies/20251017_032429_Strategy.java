...
public class Strategy implements IStrategy {
    ...
    private double advisoryWeight = 8.0; // weight constant: -10 to +10, negative biases towards SELL
    ...
    public void onBar(Instrument instrument, Period period, IBar askBar, IBar bidBar) throws JFException {
        if (instrument == Instrument.EURUSD && period == Period.ONE_MIN) {
            ...
            // Random decision to place order on this M1 bar: ~10% chance per bar (~6 per hour avg, random timing)
            // Bias slightly towards more frequent in down hours via advisoryWeight (negative = more orders)
            double orderProb = 0.05 + (Math.abs(advisoryWeight) / 100.0); // Base 10%, up to 20% if |weight| high
            if (rand.nextDouble() > orderProb) return;
            ...
            double tpDistance = 24 * pip; // Increased TP by 20% (was 20)
            ...
            console.getOut().println("Placed random " + cmd + " order: " + label + " (minute: " + currentMinute + ", diffPips: " + diffPips + ", probSell: " + probSell + ", weight: " + advisoryWeight + ") SL: " + sl + " TP: " + tp);
        }
    }
}